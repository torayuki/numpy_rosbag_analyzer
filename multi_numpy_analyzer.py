...
def convine_sound(video_path, wav_path, out_path, overwite=True) -> None:
...
def get_latent_state(data, key):
    if key == "beliefs":
        temp = to_np(data[key][0])
        for i in range(1, len(data[key])):
            temp = np.concatenate([temp, np.squeeze(to_np(data[key][i]), axis=0)], axis=0)
        temp = np.expand_dims(temp, axis=0)
    elif key == "prior_states":
        temp = np.expand_dims(np.expand_dims(to_np(data["posterior_states"][0]), axis=0), axis=0)
        temp = np.concatenate([temp, to_np(data[key][1])], axis=1)
    return temp


def numpy_loader(numpy_dir, dataset_path):
    start_time = 0
    stamps = {}
    observations = {}
    latent_state = {}
    ...
    dir_path = os.path.join(dataset_path, numpy_dir)
    ...
    for i in range(n_frame):
        ...
        for obs_key in observation.keys():
            if not obs_key in observations.keys():
                observations[obs_key] = to_np(observation[obs_key])
            else:
                observations[obs_key] = np.concatenate([observations[obs_key], to_np(observation[obs_key])], axis=0)
...
def plot_ls_traj(data, ax, t, dim=2, lim=10):
...
def plot_ts_line(data, x_key, y_key, ax, t, length=10):
    ...
    ax.set_xticks(np.arange(np.round(min(data_x) - 0.1, 1), np.round(max(data_x) + 0.1, 1), 0.1))
    ax.set_ylim(0, 0.2)
...
def plot_stamp_obs_action_ls(stamps, observations, actions, latent_state, pca_belief, pca_post_mean, plot_dir, file_name, n_frame=None, pca_dim=2):
    ...
    def plot(t):
        ...
        mlsp = sound_todb(observations["sound"][t])
        ...
        # latent state
        plot_ls_traj(clip_belief, ax=ax3, t=t, dim=pca_dim, lim=2)
        plot_ls_traj(clip_prior_states, ax=ax7, t=t, dim=pca_dim, lim=5)
...
def whole_process(numpy_dir, dataset_path, results_path, pca_belief_path, pca_post_mean_path, n_frame, device):
    save_folder_name = os.path.join(results_path, numpy_dir)
    os.makedirs(save_folder_name, exist_ok=True)
    
    pca_belief = pk.load(open(pca_belief_path, "rb"))
    pca_post_mean = pk.load(open(pca_post_mean_path, "rb"))
    stamps, observations, actions, latent_state = numpy_loader(numpy_dir, dataset_path)
    plot_stamp_obs_action_ls(stamps, observations, actions, latent_state, pca_belief, pca_post_mean, save_folder_name, numpy_dir, n_frame=n_frame, pca_dim=2)
    plot_stamp_obs_action_ls(stamps, observations, actions, latent_state, pca_belief, pca_post_mean, save_folder_name, numpy_dir, n_frame=n_frame, pca_dim=3)
    
    obs_wavame = "{}/obs_wavfile.wav".format(save_folder_name)
    save_wav(sound_postprocess(observations["sound"], library="torchaudio", device=device), obs_wavame, sr=16000)
    convine_sound(os.path.join(save_folder_name, numpy_dir + "_2d" + ".mp4"), obs_wavame, os.path.join(save_folder_name, numpy_dir + "_2d_obs-sound" + ".mp4"))
    convine_sound(os.path.join(save_folder_name, numpy_dir + "_3d" + ".mp4"), obs_wavame, os.path.join(save_folder_name, numpy_dir + "_3d_obs-sound" + ".mp4"))
...
def main():
    ...
    device = "cuda:2"
    dataset_path = "numpy/2022-03-24"
    results_path = "result/2022-03-24"
    pca_belief_path = "pca/pca_belief.pkl"
    pca_post_mean_path = "pca/pca_post_mean.pkl"
    os.makedirs(results_path, exist_ok=True)
    
    if "cuda" in device:
        print("use gpu device : {}".format(device))

    numpy_dir_list = os.listdir(dataset_path)
    p = Pool(n_maxprocess if len(numpy_dir_list) > n_maxprocess else len(numpy_dir_list))

    for numpy_dir in numpy_dir_list:
        p.apply_async(whole_process, args=(numpy_dir, dataset_path, results_path, pca_belief_path, pca_post_mean_path, n_frame, device))
